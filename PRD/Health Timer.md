...
- Il faut réserver de la place pour une fonctionnalité future, qu'il ne faut pas développer pour le moment, qui servira à afficher mon rythme cardiaque, avec un bouton ON/OFF qui servira à activer ou désactiver cette fonctionnalité lorsqu'elle sera implémentée. Cette zone occupe une bande horizontale en bas de la fenêtre, visible mais entièrement grisée, avec la mention "WIP" à la place de l'affichage du rythme cardiaque. Le bouton ON/OFF est présent mais grisé et non cliquable dans la version actuelle.

# Fonctionnalités
- Afficher un unique timer à côté d'un pictogramme désignant les gestes de santé courants (hydratation, étirements, respiration). Ce timer couvre les 3 rappels à la fois — il n'y a pas 3 timers distincts. Ce timer démarre à la valeur 59 minutes et 59 secondes par défaut.
...
- Une fois qu'ils ont été démarré, les timers décrémentent progressivement, seconde après seconde, jusqu'à atteindre 0 secondes. En arrivant à 0, le pictogramme associé au timer doit clignoter : il alterne entre rouge vif (#FF0000) et blanc (#FFFFFF) à une fréquence d'une seconde (0,5 s rouge, 0,5 s blanc). Le clignotement persiste jusqu'à ce que l'utilisateur appuie sur le pictogramme pour l'arrêter, ce qui remet le timer à sa valeur initiale et arrêté.
...
- Tous les timers doivent avoir une valeur par défaut, modifiable individuellement. Il est possible de cliquer directement sur la valeur affichée du timer pour la rendre éditable inline : la valeur se transforme en un champ de saisie en place (format HH:MM:SS), sans dialog ni fenêtre modale. La valeur saisie doit être comprise entre 5 secondes minimum et 11 heures 59 minutes 59 secondes maximum.
